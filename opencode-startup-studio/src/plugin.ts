...
export const StartupStudioPlugin: Plugin = async (context, rawOptions) => {
  const dataRootOption =
    typeof rawOptions?.dataRoot === "string" && rawOptions.dataRoot.length > 0
      ? rawOptions.dataRoot
      : context.directory
  const companionUrl =
  ...
  return {
    tool: {
      ...
      startup_upsert: tool({
        description:
          "Create or replace a pool candidate. Requires edit permission on pool.json. Pass the full PoolEntry object fields.",
        args: {
          name: tool.schema.string(),
          ...
          rubric_json: tool.schema
            .string()
            .describe('JSON object {pain,payment,shelf,freshness,fit} each 0-2'),
          one_liner: tool.schema.string(),
          status: tool.schema.string().optional(),
          batch: tool.schema.string().optional(),
          first_seen: tool.schema.string().optional(),
          evaluation_json: tool.schema
            .string()
            .optional()
            .describe("JSON {pros,cons,risks,recommendation,updated_at}"),
        },
      ...
      startup_view: tool({
        ...
        async execute(args) {
          ...
          let healthy = false
          try {
            const response = await fetch(`${companionUrl}/api/health`, {
          ...
          return asJson({
            ...
            hint: healthy
              ...
              : `Companion not reachable. Start: opencode-startup-studio serve --root ${dataRoot}`,
          })
        },
      }),
    },
