...
export async function doctorStudio(input: {
  ...
  checks.push({
    ...
    source: paths.configFile,
      repair: registered ? undefined : `Run opencode-startup-studio install --scope ${paths.scope}`,
  })
  ...
  if (!digest) {
    checks.push({
      ...
      repair: `Run opencode-startup-studio install --scope ${paths.scope}`,
    })
  } else if (!marker) {
    checks.push({
      id: "skill",
  ...
  checks.push({
    ...
    status: "pass",
    message: "Startup Studio has no external domain engines (web research uses OpenCode tools)",
    source: "package",
  })
