...
export function packageRootFrom(importMetaDir: string) {
  // src/ during development, dist/ after build
...
export async function loadPackageMeta(packageRoot: string) {
  ...
  const pluginSpecifier =
    pluginExport === "." || pluginExport === "" ? packageJson.name : `${packageJson.name}/${pluginExport}`
  ...
  if (!sourceSkillDirectory.startsWith(path.resolve(packageRoot) + path.sep) && sourceSkillDirectory !== path.resolve(packageRoot)) {
