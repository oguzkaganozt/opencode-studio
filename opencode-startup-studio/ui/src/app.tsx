  ...
  return (
    <header className="flex h-10 items-center justify-between border-b border-[var(--osc-border)] px-3">
      ...
      <nav className="flex items-center gap-3 text-xs">
        <Link className="text-[var(--osc-text-muted)] hover:text-[var(--osc-text)]" to="/">
          Pool
        </Link>
        <Link className="text-[var(--osc-text-muted)] hover:text-[var(--osc-text)]" to="/rejects">
          Rejects
        </Link>
      ...
      <section className="mb-6">
        ...
        <ul className="space-y-3">
          {c.evidence.map((e) => (
            <li
              key={e.url}
              className="rounded-[var(--osc-radius-md)] border border-[var(--osc-border)] bg-[var(--osc-bg-elevated)] p-3"
            >
              <a
...
function PoolShell() {
  ...
  const listQuery = useQuery({
    queryKey: ["candidates", signalClass, verdict],
