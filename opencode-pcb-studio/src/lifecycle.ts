...
async function tsciAvailable() {
  try {
    const proc = Bun.spawn(["tsci", "--version"], { stdout: "pipe", stderr: "pipe" })
...
export async function doctorStudio(input: {
  ...
  const config = await readOpenCodeConfig(paths.configFile)
  const registered = config.exists && pluginEntries(config).some((entry) => pluginEntryMatches(entry, meta.pluginSpecifier))
  checks.push({
    ...
    repair: registered ? undefined : `Run opencode-pcb-studio install --scope ${paths.scope}`,
  })
  ...
  if (!digest) {
    checks.push({
      ...
      repair: `Run opencode-pcb-studio install --scope ${paths.scope}`,
    })
  } else if (!marker) {
    checks.push({
      id: "skill",
  ...
  const hasTsci = await tsciAvailable()
  checks.push({
    id: "domain-dependencies",
    status: hasTsci ? "pass" : "warn",
    message: hasTsci ? "tsci is available for circuit builds and exports" : "tsci not found; pcb_circuit_build and exports require tsci",
    source: "tsci --version",
    repair: hasTsci ? undefined : "Install @tscircuit/cli so the `tsci` binary is on PATH",
  })
