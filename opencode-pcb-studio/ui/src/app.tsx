...
import { api, type CircuitDiagnostics, type DiagnosticGroup, type PartSummary, type ProjectSummary } from "./api"
...
function WorkspaceBadge() {
  const { data } = useQuery({ queryKey: ["workspace"], queryFn: api.workspace })
  if (!data) return null
  return (
...
function ProjectCard({ project }: { project: ProjectSummary }) {
  return (
    <Link to={`/projects/${encodeURIComponent(project.id)}`} className="block group">
...
function ProjectsPage() {
  const { data, isLoading, error } = useQuery({ queryKey: ["projects"], queryFn: () => api.projects({ limit: 100 }) })
...
function PartDetailModal({ mpn, onClose }: { mpn: string; onClose: () => void }) {
  const { data, isLoading, error } = useQuery({ queryKey: ["part", mpn], queryFn: () => api.catalogPart(mpn) })
    ...
    es.onmessage = (msg) => {
      try {
        ...
        if (event.type === "artifacts-changed") {
          ...
          queryClient.invalidateQueries({ queryKey: ["circuitJson", projectId] })
