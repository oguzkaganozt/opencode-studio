...
const STUDIO_EMPTY: Record<string, { title: string; body: string }> = {
  cad: {
    title: "CAD agent",
    body: "Ask to build or edit a part, then inspect the assembly beside this panel.",
  },
  media: {
    title: "Media agent",
    body: "Ask to generate or organize library assets, then preview them here.",
  },
  pcb: {
    title: "PCB agent",
    body: "Ask to scaffold or fix a board, then open schematic and layout here.",
  },
  startup: {
    title: "Startup agent",
    body: "Ask to evaluate or refine a candidate, then review the pool beside this panel.",
  },
}
...
function QuestionCard({ request, disabled, onSubmit }: { request: Question; disabled: boolean; onSubmit: (answers: string[][]) => void }) {
  ...
  return (
    <div data-agent-blocking className="m-3 rounded-lg border border-[var(--osc-border-strong)] bg-[var(--osc-bg-elevated)] p-3">
      <p className="mb-2 text-[11px] font-semibold tracking-[0.12em] text-[var(--osc-text-faint)] uppercase">Agent question</p>
        ...
        <div key={`${request.id}-${question.header}`} className="mb-3 last:mb-0">
          ...
          {question.custom && (
            <input
              ...
              className="mt-2 w-full rounded-md border border-[var(--osc-border)] bg-[var(--osc-bg)] px-2.5 py-1.5 text-[12px]"
            />
          )}
        </div>
      ))}
      <button
        ...
        className="mt-2 rounded-md bg-[var(--osc-primary)] px-3 py-1.5 text-[12px] font-medium text-[var(--osc-primary-fg)] disabled:opacity-35"
      >
        Send answer
      </button>
    </div>
...
export function AgentPanel({
  ...
  useEffect(() => {
    return subscribeAgentHandoff((request) => {
      setPrompt((current) => {
        ...
        if (!draft) return request.text
        if (draft === request.text) return current
        return `${current.trimEnd()}\n\n${request.text}`
      })
      if (request.focus) setFocusToken((token) => token + 1)
    })
  }, [])
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      // Keep panel open while typing or using native controls / permission UI.
      if (target?.closest("input, textarea, select, [contenteditable='true']")) return
      if (target?.closest("[data-agent-blocking]")) return
  ...
  const status = deriveAgentStatus({
    open,
    sessionsPending: open && sessionsQuery.isPending && !sessionsQuery.data,
    queryError,
  })
  const emptyCopy = STUDIO_EMPTY[studioId] ?? {
    title: `${studioLabel} agent`,
    body: "Ask for changes, then inspect the result in the studio beside this panel.",
  }
  const sendBlockedReason = !csrfToken
    ? "Can't send — refresh to restore security token"
    : authNeeded
      ? "Unlock the agent to send"
      : setupNeeded
    ...
    >
      <div className="flex h-11 shrink-0 items-center gap-2 border-b border-[var(--osc-border)] px-3">
        ...
        <select
          ...
          className="ml-auto min-w-0 max-w-[min(14rem,45%)] flex-1 truncate rounded-md border border-[var(--osc-border)] bg-[var(--osc-bg-elevated)] px-2 py-1 text-[12px]"
        ...
        <button
          ...
          className="grid size-7 shrink-0 place-items-center rounded-md border border-[var(--osc-border)] text-base hover:bg-[var(--osc-surface)] disabled:opacity-35"
        ...
        <button
          type="button"
          onClick={onClose}
          className="ml-0.5 grid size-7 shrink-0 place-items-center rounded-md text-[14px] text-[var(--osc-text-muted)] hover:bg-[var(--osc-surface)]"
      ...
      >
        {sessionsQuery.isLoading && <p className="p-4 text-[13px] text-[var(--osc-text-muted)]">Starting OpenCode…</p>}
        {setupNeeded && (
          <div className="m-3 rounded-lg border border-[var(--osc-warning)]/40 bg-[var(--osc-warning-bg)] p-3 text-[13px] text-[var(--osc-text)]">
            Set <code className="font-mono text-[12px]">OPENCODE_STUDIO_PASSWORD</code> and restart the host to enable the agent remotely.
          </div>
        )}
        {authNeeded && (
          <form
          ...
          >
            <label className="text-[12px] font-medium" htmlFor="agent-password">
              Agent password
            </label>
            <div className="mt-2 flex gap-2">
              <input
                ...
                className="min-w-0 flex-1 rounded-md border border-[var(--osc-border)] bg-[var(--osc-bg)] px-2.5 py-1.5 text-[13px]"
              />
              <button className="rounded-md bg-[var(--osc-primary)] px-3 text-[12px] text-[var(--osc-primary-fg)]" type="submit">
                Unlock
              </button>
            </div>
          </form>
        ...
        {queryError && !authNeeded && !setupNeeded && (
          <p className="m-3 rounded-lg border border-[var(--osc-error)] bg-[var(--osc-error-bg)] p-3 text-[13px] text-[var(--osc-error)]">
            {queryError.message}
          </p>
        )}
        {!queryError && stateQuery.data?.messages.length === 0 && (
          <div className="grid min-h-56 place-items-center p-8 text-center">
            <div>
              <p className="text-[14px] font-medium">{emptyCopy.title}</p>
              <p className="mt-1 text-[12px] leading-relaxed text-[var(--osc-text-muted)]">{emptyCopy.body}</p>
            </div>
          </div>
        )}
        {stateQuery.data?.messages.map((message) => {
          ...
          return (
            ...
            >
              <p className="mb-1.5 text-[11px] font-semibold tracking-[0.12em] text-[var(--osc-text-faint)] uppercase">
                {message.info.role === "user" ? "You" : "Agent"}
              </p>
              {text && <p className="whitespace-pre-wrap text-[13px] leading-[1.65] text-[var(--osc-text)]">{text}</p>}
              {tools.map((part) => (
                <details
                  key={part.id}
                  className="mt-2 rounded-md border border-[var(--osc-border)] bg-[var(--osc-bg-elevated)] px-2.5 py-2 text-[12px]"
                >
              ...
              {message.info.error?.data?.message && (
                <p className="mt-2 text-[12px] text-[var(--osc-error)]">{message.info.error.data.message}</p>
              )}
            </article>
          )
        })}
        {stateQuery.data?.permissions.map((permission) => (
          <div
            key={permission.id}
            data-agent-blocking
            className="m-3 rounded-lg border border-[var(--osc-warning)]/40 bg-[var(--osc-warning-bg)] p-3 text-[var(--osc-text)]"
          >
            <p className="text-[11px] font-semibold tracking-[0.12em] text-[var(--osc-warning)] uppercase">Permission requested</p>
            <p className="mt-1 text-[13px] font-medium">{permission.permission}</p>
            <p className="mt-1 break-all font-mono text-[11px] text-[var(--osc-text-muted)]">{permission.patterns.join(", ")}</p>
            <div className="mt-3 flex flex-wrap gap-1.5">
              {(["once", "always", "reject"] as const).map((reply) => (
                <button
                  ...
                  className="rounded-md border border-[var(--osc-border-strong)] bg-[var(--osc-bg-elevated)] px-2.5 py-1 text-[12px] font-medium hover:bg-[var(--osc-surface)] disabled:opacity-40"
                >
                  {PERMISSION_LABELS[reply]}
                </button>
              ))}
            </div>
          </div>
        ...
        {busy && (
          <p className="px-4 py-3 text-[11px] font-medium tracking-[0.1em] text-[var(--osc-text-faint)] uppercase">Agent is working…</p>
        )}
      </div>
      ...
      >
        <textarea
          ...
          onKeyDown={(event) => {
            if (event.key === "Enter" && !event.shiftKey) {
              event.preventDefault()
              if (prompt.trim() && !sendPrompt.isPending && csrfToken && !authNeeded && !setupNeeded) {
          ...
          className="block w-full resize-none rounded-lg border border-[var(--osc-border)] bg-[var(--osc-bg)] px-3 py-2 text-[13px] leading-relaxed outline-none focus:border-[var(--osc-border-strong)] disabled:opacity-50"
        />
        <div className="mt-2 flex items-center justify-between gap-2">
          <span className="min-w-0 text-[11px] text-[var(--osc-text-faint)]">
            {sendBlockedReason ?? "Enter to send · Shift+Enter for newline"}
          </span>
          {busy ? (
            <button
              type="button"
              onClick={() => action.mutate({ url: `/api/chat/sessions/${sessionID}/abort` })}
              className="shrink-0 rounded-md border border-[var(--osc-border-strong)] px-3 py-1.5 text-[12px] font-medium"
            >
              Stop
            </button>
          ) : (
            <button
              ...
              className="shrink-0 rounded-md bg-[var(--osc-primary)] px-3 py-1.5 text-[12px] font-medium text-[var(--osc-primary-fg)] disabled:opacity-35"
            >
              Send
            </button>
          )}
        ...
        {(sendPrompt.error || createSession.error || action.error) && (
          <p className="mt-2 text-[12px] text-[var(--osc-error)]">{(sendPrompt.error || createSession.error || action.error)?.message}</p>
        )}
      </form>
    </aside>
  )
