...
const toneClass: Record<BadgeTone, string> = {
  neutral: "border-[var(--osc-border)] bg-[var(--osc-bg-elevated)]/90 text-[var(--osc-text-muted)]",
...
export function Badge({ className, tone = "neutral", ...props }: HTMLAttributes<HTMLSpanElement> & { tone?: BadgeTone }) {
  return (
    <span
      className={cn(
        "inline-flex items-center rounded-full border px-2 py-0.5 font-mono text-[10px] tracking-wide uppercase backdrop-blur-sm",
        toneClass[tone],
        className,
      )}
      {...props}
