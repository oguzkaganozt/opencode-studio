...
import { artifactUrl, type DesignSummary, fetchStudio, listDesigns, readDesign, renderUrl } from "./api"
...
function StudioBar() {
  const studio = useQuery({ queryKey: ["studio"], queryFn: fetchStudio, staleTime: 60_000 })
  return (
...
function DesignWorkspace({ designId }: { designId?: string }) {
  ...
  useEffect(() => {
    if (!designId && designs.length > 0 && !localParts) {
      navigate(`/designs/${designs[0]!.id}`, { replace: true })
    }
      ...
      <div className="relative min-h-0 min-w-0 flex-1 bg-[var(--osc-canvas-bg)]">
        <div className="absolute top-2 left-2 z-10 flex flex-wrap items-center gap-1">
          ...
          <select
            ...
            onChange={(event) => {
              setLocalParts(null)
              if (event.target.value) navigate(`/designs/${event.target.value}`)
            }}
          >
