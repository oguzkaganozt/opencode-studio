...
export async function doctorStudio(input: {
  ...
  checks.push({
    ...
    repair: registered ? undefined : `Run opencode-cad-studio install --scope ${paths.scope}`,
  })
  ...
  if (!digest) {
    checks.push({
      ...
      repair: `Run opencode-cad-studio install --scope ${paths.scope}`,
    })
  } else if (!marker) {
    checks.push({
      id: "skill",
  ...
  checks.push({
    ...
    status: hasUv ? "pass" : "warn",
    message: hasUv ? "uv is available for forge and build123d-mcp" : "uv not found; forge builds and build123d-mcp require uv",
    source: "uv --version",
    repair: hasUv ? undefined : "Install uv (https://docs.astral.sh/uv/) then run uv sync --project forge",
  })
