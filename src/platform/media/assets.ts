...
export async function importMediaAsset(input: {
  ...
  filePath: string
  outputRoot: string
  /** Default directory for imports (workspace/media) */
  mediaDir: string
  signal: AbortSignal
  ask: AskPermission
  outputPath?: string
  ...
  if (isInside(input.outputRoot, inspected.filePath)) {
    return {
      filePath: inspected.filePath,
      bytes: inspected.bytes,
      mime: inspected.mime,
      modality: inspected.modality,
  ...
  const defaultName = `${stem}-${randomUUID().slice(0, 8)}.${inspected.extension}`
  const outputPath = input.outputPath
    ? path.isAbsolute(input.outputPath)
      ? input.outputPath
      : path.resolve(input.outputRoot, input.outputPath)
    : path.join(input.mediaDir, defaultName)
