...
async function browserSmoke(base: string) {
  ...
  try {
    ...
    await page.locator(".osc-advanced > summary").click()
    await page.getByRole("button", { name: "Repair installation" }).waitFor()
    ...
    assert(mobileDrawerWidth >= 359, `360 settings: drawer should be full width, got ${mobileDrawerWidth}`)
    await assertNoHorizontalScroll(page, "360 settings")
    await page.goto(`${base}/studio`, { waitUntil: "networkidle" })
    await page.waitForSelector("text=OpenCode")
