...
from cad_runtime._design_audit_subprocess import _extract_params, run_audit
...
def _security_config() -> dict:
    ...
    import cad_runtime.security as sec
...
def design_audit(session, epsilon: float = 0.1, max_params: int = 8) -> str:
    ...
    try:
        ...
        try:
            proc = subprocess.run(
                [sys.executable, "-m", "cad_runtime._design_audit_subprocess", in_json, out_json],
                capture_output=True,
                text=True,
