...
- **Region** — toolbar **Face | Rect | Free** (default **Face**). Requires face-split GLB from `design_build` (`face_*` meshes). **Face**: tap commits the whole face (`kind=face`, mesh boundary outline; plane faces get `plane-projected` fill + axes). **Rect**: drag opposite corners on a **planar** face (toast otherwise); live W×H on canvas; lift commits a 4-corner zone (`kind=rect`, `size_mm`, `frame=viewer-plane` — sides follow viewer UV, not necessarily design edges). After commit, HUD **W / H** fields override size (center fixed) and set `size.quality=construction`. Drag-only keeps `quality=mesh-approx`. Corners use the same mesh vertex snap as Pick. **Free**: freehand closed zones on one locked face; loop back to start to keep; open lift discards. Cap 5 regions. Multi-face intent = multiple regions. **Clear** removes regions only.
