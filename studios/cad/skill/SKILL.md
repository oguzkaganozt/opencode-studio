...
2. Phase 1 — `cad_ir_docs` if needed → write each part via `cad_ir_apply` → `cad_design_build` (compiles IR)  
...
- `cad_design_create(id, parts[{id, qty}], params, acceptance)` — locked contract (schema 1, no contractHash). `qty`: `1` one body, `2` one source + YZ mirror. New parts default to IR. You model every part.
