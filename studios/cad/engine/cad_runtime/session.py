...
from cad_runtime.security import (
...
class Session:
    ...
    def _inject_builtins(self) -> None:
        ...
        def save_json(name: str, obj: Any) -> str:
            ...
            out_dir = Path(tempfile.gettempdir()) / "studio-cad-runtime" / f"pid-{os.getpid()}"
        ...
        def measure(shape: Any = None, density: float = 0.0, material: str = "") -> dict:
            ...
            from cad_runtime.tools._bounded import run_bounded_shape_op
            from cad_runtime.tools.measure import _measure_report, _resolve_density
        ...
        def cross_sections(shape: Any = None, axis: str = "Z", num_slices: int = 10) -> list:
            ...
            from cad_runtime.tools._bounded import run_bounded_shape_op
            from cad_runtime.tools.cross_sections import _cross_sections_report
        ...
        def find_bored_bosses(shape: Any = None) -> list:
            ...
            central bore.
            """
            from cad_runtime.tools.find_features import _find_bored_boss_candidates
        ...
        def find_countersinks(shape: Any = None) -> list:
            ...
            from cad_runtime.tools.recognizers.countersink import recognise_countersinks
        ...
        def align_check(a: Any, b: Any, axis: str = "Z", mode: str = "flush") -> dict:
            ...
            from cad_runtime.tools.align_check import _align_check
    ...
    def _sync_registry_into_namespace(self) -> None:
        """Bind show()/import registry shapes into the execute() Python namespace."""
        self.namespace["cad_objects"] = self.objects

        def cad_object(name: str):
            if name not in self.objects:
                raise KeyError(
                    f"Unknown cad object {name!r}. Registered: {sorted(self.objects)}"
                )
            return self.objects[name]

        self.namespace["cad_object"] = cad_object
        for name, shape in list(self.objects.items()):
