...
from cad_runtime.tools.validate import _gate_report, _resolve_shape
...
def _check_features(session, object_name: str, features: list, out: list) -> None:
    from cad_runtime.tools.find_features import find_bosses, find_hole_patterns, find_holes
    from cad_runtime.tools.recognizers.countersink import find_countersinks
...
def _check_parameters(session, params_spec: list, out: list) -> None:
    from cad_runtime._design_audit_subprocess import _extract_params
    from cad_runtime.tools.design_audit import _assemble
...
def verify_spec(session, spec: str = "", spec_path: str = "", object_name: str = "") -> str:
    ...
    try:
        if "envelope_mm" in data or "volume_mm3" in data:
            from cad_runtime.tools.measure import measure as _measure
