...
from cad_runtime.tools._budget import op_budget
from cad_runtime.tools.diff import _shape_diag
from cad_runtime.tools.measure import _center_of_mass
...
def _surface_compare_in_process(sa, sb, deadline=None) -> dict:
    from cad_runtime._shape_compare_subprocess import compare_shapes
...
def _surface_compare_bounded(session, sa, sb) -> dict:
    from cad_runtime.tools.export import _write_step
        ...
        try:
            proc = subprocess.run(
                [
                    sys.executable,
                    "-m",
                    "cad_runtime._shape_compare_subprocess",
                    a_step,
                    b_step,
                    out_json,
