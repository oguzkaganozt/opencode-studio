...
from cad_runtime.tools._marshal import marshal_render_drawing, marshal_render_view
from cad_runtime.worker import WorkerSession
...
mcp = FastMCP("studio-cad-runtime", instructions=_INSTRUCTIONS, stateless_http=True)
...
# Live-session viewer publisher (cad_runtime.viewer.ViewerPublisher), or None
...
def start_viewer(socket_path: str):
    ...
    """
    global _viewer
    from cad_runtime.viewer import ViewerPublisher
...
@mcp.tool(annotations=_MUTATING)
def studio_build(design_dir: str) -> str:
    """Build a CAD Studio design directory in-process (deterministic parts/*.py → STEP/STL/GLB + manifest). Same runtime as session tools; disk sources are the source of truth. Returns JSON {ok, exitCode, designDir, manifestPath, stdout, stderr}."""
...
def repair_hints(error_text: str) -> str:
    ...
    from cad_runtime.tools.repair_hints import repair_hints as _repair_hints
...
def repair_advice(error_text: str = "", goal: str = "", context: str = "") -> str:
    ...
    from cad_runtime.tools.repair_advice import repair_advice as _repair_advice
...
def version() -> str:
    ...
    from cad_runtime.tools.version import version_info
...
def build123d_quickref() -> str:
    ...
    from cad_runtime.quickref import build_quickref_text
...
def build123d_selectors_cookbook() -> str:
    ...
    from cad_runtime.selectors_cookbook import build_selectors_cookbook_text
...
def build123d_drafting_cookbook() -> str:
    ...
    from cad_runtime.drafting_cookbook import build_drafting_cookbook_text
...
def build123d_presentation_cookbook() -> str:
    ...
    from cad_runtime.presentation_cookbook import build_presentation_cookbook_text
...
def build123d_bd_warehouse() -> str:
    ...
    from cad_runtime.bd_warehouse_resource import build_bd_warehouse_text
