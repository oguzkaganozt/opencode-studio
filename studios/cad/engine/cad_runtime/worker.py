...
from cad_runtime.tools._budget import OP_BUDGET_FLOOR_S
...
def _build_session(
    ...
    library_index = None
    if library_path:
        from cad_runtime.tools.library import _LibraryIndex
...
def _op_search_library(session: Any, args: dict, library_index: Any) -> Any:
    ...
    from cad_runtime.tools.library import search_library
...
def _op_load_part(session: Any, args: dict, library_index: Any) -> Any:
    if library_index is None:
        return "No part library configured."
    from cad_runtime.tools.library import load_part
...
def _op_view_axes(session: Any, args: dict, library_index: Any) -> Any:
    ...
    from cad_runtime.tools.view_axes import view_axes
...
def _op_render_drawing(session: Any, args: dict, library_index: Any) -> Any:
    ...
    from cad_runtime.tools.render_drawing import render_drawing
...
def _op_pull_viewer_deltas(session: Any, args: dict, library_index: Any) -> Any:
    ...
    meshes: dict = {}
    if upsert_names:
        from cad_runtime.tools.render import _QUALITY, _tessellate_shapes_bounded
...
class WorkerSession:
    ...
    def _do_call(self, op: str, args: dict, timeout: int) -> Any:
        ...
        if not self._conn.poll(timeout):
            ...
            from cad_runtime.security import ExecutionTimeout
    ...
    @_op(_tool(f"{_T}.wall_at:wall_at"), _GEOMETRY_TIMEOUT)
    def wall_at(self, object_name: str = "", at_mm: str = "", direction: str = "") -> str:
        raise NotImplementedError
