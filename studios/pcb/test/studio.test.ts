...
import { inspectCatalog } from "../catalog"
import {
  elementTypeCounts,
  inspectCircuitJson,
...
import { validateProjectName } from "../scaffold"
...
import { checkCadAssetHealth, preferKicadStepModels } from "../viewer/src/cad-models"
import { decodeProjectId, encodeProjectId } from "../workspace"

function testApp(workspaceRoot: string) {
  const app = new Hono()
  app.use("*", async (ctx, next) => {
    if (!allowedHost(ctx.req.header("host"), "127.0.0.1", 4174)) {
  ...
  app.route("/api", createPcbApi(workspaceRoot))
  return app
}

async function api(app: ReturnType<typeof testApp>, path: string) {
...
describe("isInside", () => {
  ...
  test("rejects exact parent path '..'", () => {
    expect(isInside("/ws/project", "/ws")).toBe(false)
    expect(isInside("/ws/project", path.resolve("/ws/project", ".."))).toBe(false)
  })
})
