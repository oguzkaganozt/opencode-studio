...
/**
 * Validate that `filePath` is a regular file inside the workspace (rejects symlinks).
 */
export async function assertWorkspaceFile(workspaceRoot: string, filePath: string): Promise<void> {
  ...
  if (info.isSymbolicLink()) throw new Error(`Symlinks are not allowed: ${filePath}`)
