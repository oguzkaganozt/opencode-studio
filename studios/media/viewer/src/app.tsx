...
function Shell() {
  ...
  const version = useQuery({ queryKey: ["media", "version"], queryFn: getVersion, retry: false, refetchInterval: 6 * 60 * 60 * 1000 })
  return (
    <div className="studio-shell" data-studio="media">
      <div className="grain" aria-hidden="true" />
      <header className="topbar">
        <Link to={studioHref()} className="brand" aria-label="OpenCode Media Studio Library">
...
function AssetCard({ asset, index }: { asset: Asset; index: number }) {
  ...
  return (
    <article className="asset-card" style={{ animationDelay: `${Math.min(index, 8) * 40}ms` }}>
      <Link to={studioHref(`assets/${asset.ref}`)} className="asset-card-link">
...
function AssetPage() {
  ...
  return (
    <section className="detail-page reveal">
      <Link to={studioHref()} className="back-link">
...
function NotFound() {
  return (
    <div className="empty-state">
      ...
      <Link to={studioHref()}>Return to Library</Link>
    </div>
  )
}
