...
function positiveInteger(value: unknown, fallback: number, name: string) {
  ...
  if (typeof output !== "number" || !Number.isSafeInteger(output) || output <= 0) {
    throw new Error(`opencode-media-studio: ${name} must be a positive integer`)
  }
  return output
}

function libraryRootOption(value: unknown) {
  if (value === undefined) return undefined
  if (typeof value !== "string" || !path.isAbsolute(value) || value.includes("\0")) {
    throw new Error("opencode-media-studio: libraryRoot must be an absolute path")
  }
  return value
}
...
function options(input: PluginOptions | undefined): Options {
  const downloadHosts = Array.isArray(input?.downloadHosts)
    ...
    : ["fal.media"]
  if (downloadHosts.length === 0) throw new Error("opencode-media-studio: downloadHosts must contain at least one hostname")
  return {
...
export function createMediaStudioPlugin(dependencies: MediaStudioPluginDependencies = {}): Plugin {
  ...
  return async (context, rawOptions) => {
    ...
    const workspaceRoot = await canonicalStudioRoot(context.directory)
    const library = await initializeLibrary({ root: config.libraryRoot, resolveUsername: dependencies.resolveUsername })
    ...
    return {
      ...
      tool: {
        [READ_TOOL_NAME]: tool({
          ...
          args: {
            filePath: tool.schema.string().describe("Validated absolute or Library-relative managed asset path"),
          },
        ...
        chatgpt_image_generate: tool({
          ...
          args: {
            ...
            outputPath: tool.schema
              .string()
              .optional()
              .describe("Optional PNG filename or path in the current user's Library images directory"),
        ...
        media_import: tool({
          description: "Copy a server-local image, audio, or video file into the current user's personal Library space.",
          args: {
          ...
          async execute(args, toolContext) {
            const media = await importMediaAsset({
              ...
              outputDirectory: library.personal,
              signal: toolContext.abort,
              ask: toolContext.ask,
            })
            return {
        ...
        media_list: tool({
          description: "Scan managed media assets in the shared Library filesystem.",
          args: {
            modality: tool.schema.enum(["image", "audio", "video"]).optional(),
            user: tool.schema.string().optional(),
            scope: tool.schema.enum(["personal", "shared"]).optional(),
          ...
          async execute(args) {
            const assets = await scanLibrary({
              ...
              user: args.user,
              scope: args.scope as "personal" | "shared" | undefined,
        ...
        media_info: tool({
          description: "Inspect current filesystem and detected-format information for one managed Library asset.",
          args: {
            filePath: tool.schema.string().describe("Validated absolute or Library-relative managed asset path"),
          },
          async execute(args) {
        ...
        media_probe: tool({
          description: "Inspect a managed Library asset with ffprobe without persisting probe data.",
          args: {
            filePath: tool.schema.string().describe("Validated absolute or Library-relative managed asset path"),
          },
          async execute(args, toolContext) {
        ...
        media_convert: tool({
          ...
          args: {
            filePath: tool.schema.string().describe("Managed input asset path inside the Library root"),
            preset: tool.schema.enum(["video-mp4", "video-webm", "audio-mp3", "audio-wav", "image-png", "image-webp"]),
            outputPath: tool.schema
              .string()
              .optional()
              .describe("Optional filename or path in the current user's output modality directory"),
        ...
        media_download: tool({
          description: "Download generated media into the current user's personal Library space.",
          args: {
            url: tool.schema.string().url().describe("HTTPS media URL returned by a generation provider"),
            outputPath: tool.schema
              .string()
              .optional()
              .describe("Optional filename or path in the current user's matching modality directory"),
          },
          async execute(args, context) {
