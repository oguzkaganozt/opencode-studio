...
- **OpenCode-first host:** lifecycle is owned by `opencode serve`. Plugin ensure-hosts Viewer in-process when a directory Instance loads (OpenCode loads plugins per directory — bare serve alone does not). Host **rebinds** workspace when another directory Instance loads or `session.created`/`session.updated` fires (no first-directory pin). Agent proxy prefers the client request directory; falls back to active host workspace. Attaches to `context.serverUrl`, reverse-proxies parent at `/` for Agent. Studio **never** spawns OpenCode; no user-facing `serve`/host systemd. Bind follows parent / `OPENCODE_STUDIO_*`; port `OPENCODE_STUDIO_PORT` (default 4173; busy fails, no ephemeral). Ensure reuses only when **this process** owns the host. Opt out: `OPENCODE_STUDIO_AUTOSTART=0`.
