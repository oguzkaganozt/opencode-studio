...
import { atomicWriteOpenCodeConfig, configWithPlugins, pluginEntries, readOpenCodeConfig } from "./opencode-config"
import { loadPackageMeta, skillDigest } from "./package-meta"
...
export function resolveLifecyclePaths(input: {
  ...
  if (input.configHome) {
    ...
    if (getuid?.() === 0) {
      throw new Error("Refusing implicit root OpenCode config; run as the OpenCode user or pass --config-home")
    }
...
function pluginEntryPath(entry: unknown): string | undefined {
  if (typeof entry === "string") return entry
  if (Array.isArray(entry) && typeof entry[0] === "string") return entry[0]
  return
}

/** Match npm specifier or legacy absolute/file URL paths into this package's plugin entry. */
export function pluginEntryMatches(entry: unknown, pluginSpecifier: string, packageName: string) {
  const value = pluginEntryPath(entry)
  if (!value) return false
  if (value === pluginSpecifier || value === packageName) return true
  const bare = value.startsWith("file://") ? value.slice("file://".length) : value
  const normalized = bare.replaceAll("\\", "/")
  return (
    normalized.endsWith(`/${packageName}/dist/plugin.js`) ||
    normalized.endsWith(`/${packageName}/src/plugin.ts`) ||
    normalized === `${packageName}/dist/plugin.js` ||
    normalized === `${packageName}/src/plugin.ts`
  )
}
...
async function readMarker(markerFile: string): Promise<ManagedMarker | null> {
  try {
    ...
    if (typeof raw.studioId !== "string" || typeof raw.packageVersion !== "string" || typeof raw.digest !== "string") {
      return null
    }
...
export async function installStudio(input: {
  ...
  const alreadyRegistered = plugins.some((entry) => pluginEntryMatches(entry, meta.pluginSpecifier, meta.packageName))
  ...
  try {
    ...
    if (updatedConfigText !== config.text) {
      await atomicWriteOpenCodeConfig(paths.configFile, updatedConfigText, config.exists ? config.text : "")
    }
  } catch (error) {
...
export async function removeStudio(input: {
  ...
  const hasRegistration = currentPlugins.some((entry) => pluginEntryMatches(entry, meta.pluginSpecifier, meta.packageName))
  const plugins = currentPlugins.filter((entry) => !pluginEntryMatches(entry, meta.pluginSpecifier, meta.packageName))
  ...
  try {
    // Drop skill and/or marker for this studio. Marker-only leftovers (skill
    // deleted by hand) must still be cleaned so remove never orphans ownership.
    if (marker) {
      if (existingDigest) {
  ...
  if (stagedSkill) await rm(skillBackup, { force: true })
  if (stagedMarker) {
...
export async function doctorStudio(input: {
  ...
  const config = await readOpenCodeConfig(paths.configFile)
  const registered =
    config.exists && pluginEntries(config).some((entry) => pluginEntryMatches(entry, meta.pluginSpecifier, meta.packageName))
  checks.push({
    id: "plugin-registration",
    status: registered ? "pass" : "warn",
    message: registered ? `Plugin registered as ${meta.pluginSpecifier}` : "Plugin not registered in OpenCode config",
    source: paths.configFile,
    repair: registered ? undefined : `Run opencode-media-studio install --scope ${paths.scope}`,
  })
  ...
  if (!digest) {
    checks.push({
      ...
      repair: `Run opencode-media-studio install --scope ${paths.scope}`,
    })
  } else if (!marker) {
    checks.push({
      id: "skill",
  ...
  if (input.companionUrl) {
    try {
      ...
      checks.push({
        ...
        message: response.ok ? `Companion reachable at ${input.companionUrl}` : `Companion health failed (${response.status})`,
  ...
  checks.push({
    ...
    message: hasFfmpeg
      ? "ffmpeg is available for media_convert/probe/trim operations"
      : "ffmpeg not found; convert/probe/trim tools require ffmpeg/ffprobe",
    source: "ffmpeg -version",
    repair: hasFfmpeg ? undefined : "Install ffmpeg and ffprobe so they are on PATH",
  })
