...
import plugin, { AnthropicNativeMediaProviderPlugin, createMediaStudioPlugin } from "../src/plugin"
...
function personal(modality: "images" | "audio" | "video", filename: string) {
  return path.join(libraryRoot, "users", userInfo().username, modality, filename)
}
...
describe("combined video plugin", () => {
  ...
  test("validates maxNativeBytes and the configured Library root", async () => {
    ...
    await expect(hooks({ libraryRoot: "relative" })).rejects.toThrow("libraryRoot must be an absolute path")
  ...
  test("rejects unmanaged and outside-symlink paths before permission", async () => {
    ...
    await expect(value.tool!.read_media.execute({ filePath: outsideFile }, context)).rejects.toThrow("not a managed Library asset")
    await expect(value.tool!.read_media.execute({ filePath: "../outside.mp4" }, context)).rejects.toThrow("not a managed Library asset")
    await expect(value.tool!.read_media.execute({ filePath: linked }, context)).rejects.toThrow("regular managed")
  ...
  test("generates a validated PNG with ChatGPT subscription auth", async () => {
    ...
    expect(JSON.parse(String(listed))[0]).toMatchObject({ filePath: generatedPath, modality: "image", scope: "personal" })
